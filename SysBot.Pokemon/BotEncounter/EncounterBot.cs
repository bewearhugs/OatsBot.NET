    ...
    public class EncounterBot : PokeRoutineExecutor
    {
        ...
        private async Task<int> StepUntilEncounter(CancellationToken token)
        {
            ...
            while (!token.IsCancellationRequested && Config.NextRoutineType == PokeRoutineType.EncounterBot)
            {
                ...
                {
                    short switchDirection = 1;
                    switch (Hub.Config.Encounter.InvertStartingMovement)
                    {
                        case true:
                            switchDirection = -1; break;
                        case false:
                            switchDirection = 1; break;

                    }

                    int directionDelay = Hub.Config.Encounter.SwitchDirectionDelay;
                    switch (Hub.Config.Encounter.EncounteringType)
                    {
                        case EncounterMode.VerticalLine:
                            await SetStick(LEFT, 0, (short)(-30000 * switchDirection), directionDelay, token).ConfigureAwait(false);
                        ...
                        case EncounterMode.HorizontalLine:
                            await SetStick(LEFT, (short)(-30000 * switchDirection), 0, directionDelay, token).ConfigureAwait(false);
                            await SetStick(LEFT, 0, 0, 0_100, token).ConfigureAwait(false); // reset
                            ...
                            await SetStick(LEFT, 0, 0, 0_100, token).ConfigureAwait(false); // reset
                            break;
                        case EncounterMode.LeftToRightDiagonal:
                            await SetStick(LEFT, (short)(30000 * switchDirection), (short)(-30000 * switchDirection), directionDelay, token).ConfigureAwait(false);
                            await SetStick(LEFT, 0, 0, 0_100, token).ConfigureAwait(false); // reset
                            ...
                            await SetStick(LEFT, 0, 0, 0_100, token).ConfigureAwait(false); // reset
                            break;
                        case EncounterMode.RightToLeftDiagonal:
                            await SetStick(LEFT, (short)(-30000 * switchDirection), (short)(-30000 * switchDirection), directionDelay, token).ConfigureAwait(false);
                            await SetStick(LEFT, 0, 0, 0_100, token).ConfigureAwait(false); // reset
